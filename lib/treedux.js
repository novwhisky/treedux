(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["treedux"] = factory();
	else
		root["treedux"] = factory();
})(this, function() {
...
exports.shiftKeyPath = shiftKeyPath;
exports.scaffold = scaffold;
exports.isObject = isObject;
exports.mergeDeep = mergeDeep;
exports.selectorFromKeyPath = selectorFromKeyPath;
...
function shiftKeyPath(keyPath) {
  var _keyPath$split = keyPath.split('.'),
      _keyPath$split2 = _toArray(_keyPath$split),
      key = _keyPath$split2[0],
      path = _keyPath$split2.slice(1);
...
function scaffold(keyPath) {
  var targetValue = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  var stateCopy = {};

  var _shiftKeyPath = shiftKeyPath(keyPath),
      _shiftKeyPath2 = _slicedToArray(_shiftKeyPath, 2),
      key = _shiftKeyPath2[0],
      path = _shiftKeyPath2[1];
...
function selectorFromKeyPath(keyPath) {
  if (Array.isArray(keyPath)) {
    var _keyPath2 = _toArray(keyPath),
        key = _keyPath2[0],
        path = _keyPath2.slice(1);
    ...
    if (path && path.length > 0) {
      return function (state) {
        return selectorFromKeyPath(path)(selector(state));
      };
...
function deriveChildContext(key, path, state, action) {
  var ctx = void 0;
  var ctxAction = _extends({}, action, { type: path });
 ...
 * @returns {function(*=, *=)}
 */
function createReducerTree(children, ownReducer) {
  ...
  var finalReducer = function finalReducer(state, action) {
    var _shiftKeyPath = (0, _util.shiftKeyPath)(action.type),
        _shiftKeyPath2 = _slicedToArray(_shiftKeyPath, 2),
        reducerKey = _shiftKeyPath2[0],
        path = _shiftKeyPath2[1];

    var childKeys = Object.keys(children);
    var childState = {};
    childKeys.forEach(function (key) {
      var childReducer = children[key];

      var _deriveChildContext = deriveChildContext(key, path, state, action),
          _deriveChildContext2 = _slicedToArray(_deriveChildContext, 2),
          ctx = _deriveChildContext2[0],
          ctxAction = _deriveChildContext2[1];

      childState[key] = childReducer(ctx, ctxAction);
    });

    var ownState = ownReducer(state, action);

    return _extends({}, childState, ownState);
  };
...
var _actionCreator = __webpack_require__(25);

Object.defineProperty(exports, 'bindActionCreatorTree', {
  enumerable: true,
  get: function get() {
    return _actionCreator.bindActionCreatorTree;
  }
});

var _connectContext = __webpack_require__(41);

Object.defineProperty(exports, 'context', {
  enumerable: true,
  get: function get() {
    return _connectContext.context;
  }
});
Object.defineProperty(exports, 'connectToContext', {
  enumerable: true,
  get: function get() {
    return _connectContext.connectToContext;
  }
});
...
Object.defineProperty(exports, 'createReducerTree', {
  enumerable: true,
  get: function get() {
    return _reducer.createReducerTree;
  }
});
Object.defineProperty(exports, 'reduceChildren', {
  enumerable: true,
  get: function get() {
    return _reducer.reduceChildren;
  }
});
...
exports.actionCreatorTree = actionCreatorTree;
exports.bindActionCreatorTree = bindActionCreatorTree;
 ...
 */
function actionCreatorTree(keyPath, actionCreators) {
  var tree = {};
  Object.keys(actionCreators).forEach(function (key) {
    tree[key] = function () {
      var action = actionCreators[key].apply(actionCreators, arguments);
      var ctxKey = [keyPath, action.type].join('.');
      var ctxAction = Object.assign(action, { type: ctxKey });
      return ctxAction;
    };
  });
 ...
 * @returns {Object.<Function>}
 */
function bindActionCreatorTree(keyPath, actionCreators, dispatch) {
  var tree = actionCreatorTree(keyPath, actionCreators);
  return (0, _redux.bindActionCreators)(tree, dispatch);
}
...
exports.connectToContext = connectToContext;
...
function makeConnectContext(contextFactoryArgs) {
  var mapSliceToProps = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : function (_) {
    return {};
  };
  var mapDispatchToProps = arguments[2];

  var _contextFactoryArgs = _slicedToArray(contextFactoryArgs, 2),
      keyPath = _contextFactoryArgs[0],
      reducer = _contextFactoryArgs[1];

  // console.log(...arguments);

  var sliceSelector = (0, _util.selectorFromKeyPath)(keyPath);
  ...
  return function wrapComponent(ReactComponent) {
    ...
    function makeSelectorStateful(sourceSelector, store, sliceSelector) {
      ...
      var selector = {
        run: function runComponentSelector(props) {
          try {
            var nextProps = sourceSelector( /*sliceSelector(*/store.getState() /*)*/, store.getState(), props);
    ...
    function selectorFactory(dispatch, options) {
      ...
      return function (nextState, nextOwnProps) {
        ...
        var nextSlice = options.sliceSelector(nextState);
        ...
        var nextResult = _extends({}, nextOwnProps, nextCtx);
        ownProps = nextOwnProps;
        if (!(0, _shallowEqual2.default)(result, nextResult)) {
          result = nextResult;
    ...
    var Connect = (0, _reactRedux.connectAdvanced)(selectorFactory, options)(ReactComponent);

    var Context = function (_Connect) {
      _inherits(Context, _Connect);
      ...
      function Context(props, context) {
        ...
        var _this = _possibleConstructorReturn(this, (Context.__proto__ || Object.getPrototypeOf(Context)).call(this, props, context));
      ...
      _createClass(Context, [{
        ...
        value: function getNamespace() {
          var parentNamespace = '';
          var parentKeyPath = this.context.keyPath;
        ...
        value: function getChildContext() {
          var _context = this.context,
              ...
              storeSubscription = _context.storeSubscription;
    ...
    Context.childContextTypes = Context.contextTypes = {
      keyPath: _propTypes.string,
      store: _propTypes.object,
      storeSubscription: _propTypes.object,
      sliceSelector: _propTypes.func
    };
 ...
 * @returns {Function}
 */
function connectToContext(mapSliceToProps, mapDispatchToProps) {
  return function connectToContextFactory(ReactComponent) {
    return function wrapComponentWithContext(keyPath, reducer) {
      var connected = makeConnectContext([keyPath, reducer], mapSliceToProps, mapDispatchToProps);
      var Connect = connected(ReactComponent);
 ...
 */
function context(keyPath, reducer) {
  // keyPath, reducer
...
/*
 ...
 LICENSE file in the root directory of this source tree.
*/
var f=__webpack_require__(6),p=__webpack_require__(17);__webpack_require__(1);var r=__webpack_require__(2);
...
var _reducer2 = __webpack_require__(23);
...
var _reducer = new WeakMap();
var _children = new WeakMap();
var _hasUpdate = new WeakMap();

var ReducerLeaf = function () {
  function ReducerLeaf(key) {
    var reducer = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : function (_) {
      return {};
    };

    _classCallCheck(this, ReducerLeaf);
  ...
  _createClass(ReducerLeaf, [{
    ...
    key: 'getLeaf',
    value: function getLeaf(keyPath) {
      var kp = (0, _util.shiftKeyPath)(keyPath);

      var _kp = _slicedToArray(kp, 2),
          key = _kp[0],
          path = _kp[1];
    ...
    key: 'mount',
    value: function mount(keyPath, reducer) {

      var kp = (0, _util.shiftKeyPath)(keyPath);

      var _kp2 = _slicedToArray(kp, 2),
          key = _kp2[0],
          path = _kp2[1];

      if (key) {
        if (path && path.length > 0) {
          var child = this.getLeaf(keyPath);
          // console.log('[CHILD]', path);
          child.mount(path, reducer);
        } else {
          // console.log('[MOUNT]', key, reducer);

          var leaf = new ReducerLeaf(key, reducer);
          _children.set(this, _extends({}, this.children(), _defineProperty({}, key, leaf)));
        }
      }
    }
  }, {
    ...
    key: 'getSliceReducer',
    value: function getSliceReducer() {
      var __ignoreme = function __ignoreme(_) {
        return _;
      };

      // console.log('!', this.key());
      // console.log('#', this.childReducerMap());
      return (0, _reducer2.createReducerTree)(this.childReducerMap(), this.reducer());
    }
  }, {
    key: 'childReducerMap',
    value: function childReducerMap() {
    ...
    key: 'makeReducerTree',
    value: function makeReducerTree() {
...
function mount(store, keyPath, reducer) {
  ...
  if (true /*store.reducerLib.hasUpdate()*/) {
      var root = store.reducerLib.makeReducerTree();
      if (root) {
...
function configureStore(initialState) {
  ...
  var store = (0, _redux.createStore)(init, initialState, composeEnhancers()
...
//# sourceMappingURL=treedux.js.map