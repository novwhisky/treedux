 ...
 */
function reduceChildren(state, action) {
  var reducer = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function (_) {
    return _;
  };
  ...
  var ctx = (0, _util.getContext)(key, state);
  var ctxAction = Object.assign({}, action, { type: path });
  var slice = reducer(ctx, ctxAction);
  if (!!ctx) {
    state[key] = slice;
  }
...
function createReducerTree(children, ownReducer) {
  return function () {
    var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : (0, _util.struct)(children);
    var action = arguments[1];

    var mergeObjects = [];

    var _shiftKeyPath3 = (0, _util.shiftKeyPath)(action.type),
        _shiftKeyPath4 = _slicedToArray(_shiftKeyPath3, 1),
        reducerKey = _shiftKeyPath4[0];

    var childState = reduceChildren(state, action, children[reducerKey]);
    ...
    return Object.assign.apply(Object, [{}].concat(mergeObjects));
  };
}
...
function getIn(object, keyPath) {
  var _keyPath$split3 = keyPath.split('.'),
      _keyPath$split4 = _toArray(_keyPath$split3),
      key = _keyPath$split4[0],
      path = _keyPath$split4.slice(1);

  var ctx = object[key];
  if (path.length > 0) {
    return getIn(ctx, path.join('.'));
  } else {
    return ctx;
  }
}
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(12), __webpack_require__(36)(module)))
...
function makeConnectContext() {
  var sliceSelector = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : function (_) {
    return _;
  };
  ...
  return function wrapComponent(ReactComponent) {
    function Context(props, context) {
      ...
      var nextSliceSelector = context && context.sliceSelector ? function (state) {
        return sliceSelector(context.sliceSelector(state));
      } : function (state) {
        return sliceSelector(state);
      };

      var mapStateToProps = function mapStateToProps(state) {
        var slice = nextSliceSelector(state);
        // console.log('slice', slice);
        var mapping = mapSliceToProps(slice);
        // console.log('mapping', mapping);
        return mapping;
      };

      var Connect = (0, _reactRedux.connect)(mapStateToProps, mapDispatchToProps)(ReactComponent);
      ...
      Connect.prototype.getChildContext = function () {
        return _extends({}, context, { sliceSelector: nextSliceSelector });
      };
...
function connectToContext(mapSliceToProps, mapDispatchToProps) {
  return function connectToContextFactory(ReactComponent) {
    return function wrapComponentWithContext(sliceSelector) {
      var connected = makeConnectContext(sliceSelector, mapSliceToProps, mapDispatchToProps);
...
/**
 *
 * @param {Function<ctx>} sliceSelector (state => state.launchpad)
 * @returns {contextFactory}
 */
function context(sliceSelector) {
  if (!sliceSelector) {
...
/*
 ...
 LICENSE file in the root directory of this source tree.
*/
var f=__webpack_require__(5),p=__webpack_require__(16);__webpack_require__(1);var r=__webpack_require__(2);
...
function mount(store, keyPath, reducer) {
  ...
  store.replaceReducer((0, _reducer.createReducerTree)(store.reducerMap, init));
}

function configureStore(initialState) {
  var store = (0, _redux.createStore)(init,
